...
void cache_init(cache &cache_v, config &cache_c){
    cache_v.l1_way = cache_c.L1setsize;
    cache_v.l2_way = cache_c.L2setsize;
    cache_v.l1_offset_bit = ceil(log(cache_c.L1blocksize)/log(2));
    cache_v.l2_offset_bit = ceil(log(cache_c.L2blocksize)/log(2));
}

void cache_if(cache &cache_v, config &cache_c){
    int l1_type;
    int l2_type;

    if (cache_c.L1setsize == 1){
        l1_type = 0;
        cache_v.l1_index_bit = ceil(log(cache_v.rows1)/log(2));
        cache_v.l1_tag_bit = 32 - cache_v.l1_offset_bit - cache_v.l1_index_bit;
    }
    else if (cache_c.L1setsize == 0){
        l1_type = 2;
        cache_v.l1_tag_bit = 32 - cache_v.l1_offset_bit;
    }
    else{
        l1_type = 1;
        cache_v.l1_index_bit = ceil(log(cache_v.rows1)/log(2));
    ...
    if (cache_c.L2setsize == 1){
        l2_type = 0;
        cache_v.l2_index_bit = ceil(log(cache_v.rows2)/log(2));
        cache_v.l2_tag_bit = 32 - cache_v.l2_offset_bit - cache_v.l2_index_bit;
    }
    else if (cache_c.L2setsize == 0){
        l2_type = 2;
        cache_v.l2_tag_bit = 32 - cache_v.l2_offset_bit;
    }
    else{
        l2_type = 1;
        cache_v.l2_index_bit = ceil(log(cache_v.rows2)/log(2));
...
bitset<32>access_update(cache &cache_v,config &cache_c) {
    int L1AcceState = 0;
    int L2AcceState = 0;
    ...
    if (traces.is_open() && tracesout.is_open()) {
        while (getline(traces, line)) {
            ...
            for (int i = 0; i < cache_v.l1_offset_bit; i++) {
                offset_l1[i] = accessaddr[i];
            }
            long int offset_l1_int = offset_l1.to_ulong();
            bitset<32> index_l1(0);
            for (int i = 0; i < cache_v.l1_index_bit; i++) {
                index_l1[i] = accessaddr[i + cache_v.l1_offset_bit];
            }

            long int index_l1_trans = index_l1.to_ulong();
            int index_l1_int = index_l1_trans % cache_v.rows1;
            bitset<32> tags_l1(0);
            for (int i = 0; i < cache_v.l1_tag_bit; i++) {
                tags_l1[i] = accessaddr[i + cache_v.l1_offset_bit + cache_v.l1_index_bit];
            }
            if (accesstype.compare("R") == 0)
            {
                ...
                bool if_hit_l1 = 0;
                if (cache_c.L1setsize == 0)
                    cache_v.l1_way = cache_c.L1size * 1000 / cache_c.L1blocksize;

                for (int i = 0; i < cache_v.l1_way; i++) {
                    if (cache_v.read(index_l1_int, i, tags_l1, 1) == 1) {
                        if_hit_l1 = 1;
                        break;
                    }
                }
                if (if_hit_l1 == 1) {
                    ...
                    L1AcceState = 2;
                    cache_v.replace(index_l1_int, tags_l1, 1, addr, offset_l1_int);
                    bitset<32> offset_l2(0);
                    for (int i = 0; i < cache_v.l2_offset_bit; i++) {
                        offset_l2[i] = accessaddr[i];
                    }
                    long int offset_l2_int = offset_l2.to_ulong();
                    bitset<32> index_l2(0);
                    for (int i = 0; i < cache_v.l2_index_bit; i++) {
                        index_l2[i] = accessaddr[i + cache_v.l2_offset_bit];
                    }
                    long int index_l2_trans = index_l2.to_ulong();
                    int index_l2_int = index_l2_trans % cache_v.rows2;
                    bitset<32> tags_l2(0);
                    for (int i = 0; i < cache_v.l2_tag_bit; i++) {
                        tags_l2[i] = accessaddr[i + cache_v.l2_offset_bit + cache_v.l2_index_bit];
                    }
                    bool if_hit_l2 = 0;
                    if (cache_c.L2setsize == 0)
                        cache_v.l2_way = cache_c.L2size * 1000 / cache_c.L2blocksize;
                    for (int i = 0; i < cache_v.l2_way; i++) {
                        if (cache_v.read(index_l2_int, i, tags_l2, 2) == 1) {
                            if_hit_l2 = 1;
                            break;
                        }
                    }
                    if (if_hit_l2 == 1) {
                        ...
                        L2AcceState = 2;
                        cache_v.replace(index_l2_int, tags_l2, 2, addr, offset_l2_int);
                    }
                }
            }
            else
            {
                ...
                if (cache_c.L1setsize == 0)
                    cache_v.l1_way = cache_c.L1size * 1000 / cache_c.L1blocksize;
                for (int i = 0; i < cache_v.l1_way; i++) {
                    if (cache_v.write(index_l1_int, i, tags_l1, 1) == 1) {
                        if_hit_l1 = 1;
                        break;
                    }
                }
                if (if_hit_l1 == 1) {
                    ...
                    if (cache_c.L2setsize == 0)
                        cache_v.l2_way = cache_c.L2size * 1000 / cache_c.L2blocksize;
                    for (int i = 0; i < cache_v.l2_way; i++) {
                        if (cache_v.write(index_l2_int, i, tags_l2, 2) == 1) {
                            if_hit_l2 = 1;
                            break;
                        }
                    }
...
int main(int argc, char* argv[]){
    ...
    cache_init(my_cache, cacheconfig);
    cache_if(my_cache, cacheconfig);
    access_update(my_cache, cacheconfig);
    return 0;
}
